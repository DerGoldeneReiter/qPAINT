...
def get_tau(locs,ignore=1,**kwargs):
    ...
    ################ Extract tau_b_mean
    if np.size(tau_b_dist)==0: # If size of tau_b distribution=0 --> Set everything to zero
        tau_b_bins=0
        tau_b_cdf=0
        tau_b_mean=0
    elif np.size(tau_b_dist)<=5: # If size of tau_b distribution<=5 --> mean of distribution
        tau_b_bins,tau_b_cdf=fitfunc.ecdf(tau_b_dist) #Get ECDF
    ...
    ################ Extract tau_d_mean
    if np.size(tau_d_dist)==0: # If size of tau_b distribution=0 --> Set everything to zero
        tau_d_bins=0
        tau_d_cdf=0
        tau_d_mean=0
    elif np.size(tau_d_dist)<=5: # If size of tau_d distribution<=5 --> mean of distribution
        tau_d_bins,tau_d_cdf=fitfunc.ecdf(tau_d_dist) #Get ECDF
...
def locs2groupprops(path,ignore_dark=1):
    ...
    NoFrames=dict_list[0]['Frames']
    ############################## Define ignore_dark for different cases
    if dict_list[1]['Generated by']=='simulate_locs': # Check if locs is generated by simulate locs
    ...
    fifo.create_yaml(dict_list+ADDdict,path.replace('.hdf5','_groupprops.yaml'))

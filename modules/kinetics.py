...
def _filter(df,q):
    ...
    def drop_out(df,q):
        #### Boolean: mean_frame lower than q-percentile
        istrue=df.mean_frame<perc(df.mean_frame,q)
        #### Boolean: mean_frame higher than 100-q-percentile
        istrue=istrue | (df.mean_frame>perc(df.mean_frame,100-q))
        #### Boolean: std_frame lower than q-percentile
        istrue=istrue | (df.std_frame<perc(df.std_frame,q))
        #### Boolean: mono_tau higher than 2x50-percentile
        istrue=istrue | (df.mono_tau>(2*perc(df.mono_tau,50)))
        
        #### Drop
        df_dropped=df.drop(df.loc[istrue].index)
...
def _fit_conc(df,df_stats,use_tau_stat='mean',use_A_stat='median'):
    ...
    for expID in df_stats.index:
        df.loc[expID,'n_ac']=(df_stats.loc[expID,'fit_taud'].values*popt_mix[0])/df.loc[expID,'mono_A'].values

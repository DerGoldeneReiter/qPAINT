...
############################################################ Save filtered props as rec_array for viewing in Origin
if save_props_filter:
    X.drop('expID',axis=1,inplace=True)
    io.save_locs_recarray(X.to_records(),locs_props_yaml,path)
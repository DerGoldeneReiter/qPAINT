...
dir_name='E:/Projects/qPAINT/data/18-01-02_OwnSimulation_1ds/0ignore'
...
f1.suptitle('Own Simulation: Bright times\n'+r'$\tau_b$'+'= 100 frames\n'+
            '1 docking site per pick, ignore = 0')
# Texts
x_text=150
y_text=40
...
for i in range(0,4):
    ...
    # Text annotations
    axarr[i,0].text(x_text+300,y_text,'%d (%d)'%(tau_d_mean,tau_d_std))
...
for i in range(0,4):
    ...
    # Text annotations
    axarr[i,0].text(x_text+400,y_text,'%d (%d)'%(tau_d_mean,tau_d_std))
    axarr[i,0].text(x_text,y_text,'%d, '%(tau_ds[order[i]]),color='red')
    axarr[i,1].text(x_text,y_text,'%d (%d)'%(tau_d_cdf_mean,tau_d_cdf_std))

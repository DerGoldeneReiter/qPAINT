...
dir_name='E:/Projects/qPAINT/data/17-12-12_Simulation2x2Grid/FS_18-01-02_1ds_5ignore_bridged'
...
f1.suptitle('Picasso Simulate: Bright times\n'+r'$\tau_b$'+'= 5 s (100 frames)\n'+
            '1 docking site per pick, ignore = 5, bridged')
...
for i in range(0,4):
    # Mean & std of ac_tau
    ac_tau_mean=np.mean(fifo.read_grouprops(path[order[i]])['ac_tau_bridged'])
    ac_tau_std=np.std(fifo.read_grouprops(path[order[i]])['ac_tau_bridged'])
    # Mean & std of tau_b
    ...
    # ac_tau
    axarr[i,0].hist(fifo.read_grouprops(path[order[i]])['ac_tau_bridged'],bins='fd',label=labels[order[i]])
    # tau_b
...
f1.suptitle('Picasso Simulate: Dark time for '+r'$\tau_b$'+' = 5s'+'\n'+r'$\tau_d$'+' for '+r'$k_{on}=2e6(Ms)^{-1}$'+'\n'+
            '1 docking site per pick, ignore = 5')
# Texts
x_text=15
y_text=70

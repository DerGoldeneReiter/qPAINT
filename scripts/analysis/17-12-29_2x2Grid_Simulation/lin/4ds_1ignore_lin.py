...
dir_name='E:/Projects/qPAINT/data/17-12-12_Simulation2x2Grid/FS_18-01-03_4ds_1ignore_lin'
...
f1.suptitle('Picasso Simulate: Bright times\n'+r'$\tau_b$'+'= 5 s (100 frames)\n'+
            '4 docking sites per pick, ignore = 1, linearized')
...
for i in range(0,4):
    ...
    axarr[i,0].text(x_text,y_text+5,'%d'%(tau_cs[order[i]]),color='red')
...
# Axes limits
axarr[0,1].set_xlim([10,1e4])
axarr[0,1].set_ylim([0,32])
# Axes scale
...
for i in range(0,4):
    ...
    # Text annotations
    axarr[i,0].text(x_text+50,y_text,'%d (%d)'%(tau_d_mean,tau_d_std))
...
for i in range(0,4):
    ...
    axarr[i,0].text(x_text,y_text+5,'%1.0f'%(tau_cs[order[i]]),color='red')
...
for i in range(0,4):
    ...
    # Text annotations
    axarr[i,0].text(x_text+200,y_text,'%d (%d)'%(tau_d_mean,tau_d_std))
    axarr[i,0].text(x_text,y_text,'%d, '%(tau_ds[order[i]]/4),color='red')
    axarr[i,1].text(x_text,y_text,'%d (%d)'%(tau_d_cdf_mean,tau_d_cdf_std))
    axarr[i,2].text(x_text,y_text,'%d (%d)'%(length_mean,length_std))
    axarr[i,3].text(x_text,y_text,'%d (%d)'%(length_cdf_mean,length_cdf_std))

...
dir_name=[r'E:\Projects\qPAINT\data\18-01-03_OwnSimulation_1ds\20frames']*8
...
for i in range(0,len(path)):
    ...
    tau_c[i,1]=np.std(fifo.read_groupprops(path[i])['ac_tau'])/20
    
    tau_b_lin[i,0]=np.mean(fifo.read_groupprops(path[i])['tau_b_lin'])
    tau_b_lin[i,1]=np.std(fifo.read_groupprops(path[i])['tau_b_lin'])/20
    tau_d_lin[i,0]=np.mean(fifo.read_groupprops(path[i])['tau_d_lin'])
    tau_d_lin[i,1]=np.std(fifo.read_groupprops(path[i])['tau_d_lin'])/20
    
    p_inf[i,0]=np.mean(fifo.read_groupprops(path[i])['ac_p_inf'])
    p_inf[i,1]=np.std(fifo.read_groupprops(path[i])['ac_p_inf'])/20
    ac_tau_b[i,0]=np.mean(fifo.read_groupprops(path[i])['ac_tau_b'])
    ac_tau_b[i,1]=np.std(fifo.read_groupprops(path[i])['ac_tau_b'])/20
    ...
    ac_tau_d[i,1]=np.std(fifo.read_groupprops(path[i])['ac_tau_d'][fifo.read_groupprops(path[i])['ac_tau_d']<NoFrames[i]])/20

# Save p_inf_1 values
np.save(r'E:\Flo\repos\qPAINT\scripts\analysis\18-01-04_OwnSimulation_Screening\p_inf_1.npy',p_inf)
#%% 
# p_inf
#f=plt.figure(num=1)
#f.clear()
...
pltrange=np.arange(1,8)
# ac 
ax.errorbar(concs[pltrange]*1e9,np.divide(ac_tau_d[pltrange,0]-tau_ds[pltrange],tau_ds[pltrange])*100,yerr=np.divide(ac_tau_d[pltrange,1],tau_ds[pltrange])*100,fmt='o',label='ACF',color='blue')
# qPAINT
ax.errorbar(concs[pltrange]*1e9,np.divide(tau_d_lin[pltrange,0]-tau_ds[pltrange]/NoDocks[0],tau_ds[pltrange]/NoDocks[0])*100,yerr=np.divide(tau_d_lin[pltrange,1],tau_ds[pltrange]/NoDocks[0])*100,fmt='o',label='ECDF',color='red')
ax.axhline(0,color='black')
...
pltrange=np.arange(1,8)
# ac 
ax.errorbar(concs[pltrange]*1e9,np.divide(ac_tau_b[pltrange,0]-tau_bs[0],tau_bs[0])*100,yerr=np.divide(ac_tau_b[pltrange,1],tau_bs[0])*100,fmt='o',label='ACF',color='blue')
# qPAINT
ax.errorbar(concs[pltrange]*1e9,np.divide(tau_b_lin[pltrange,0]-tau_bs[0],tau_bs[0])*100,yerr=np.divide(tau_b_lin[pltrange,1],tau_bs[0])*100,fmt='o',label='ECDF',color='red')
# Draw zero line
ax.axhline(0,color='black')
